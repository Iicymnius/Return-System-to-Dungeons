...
quest flame_dungeon begin
	...
	function entry_setting()
		...
		when 20394.chat."Kaldýðýn Yerden Devam Et" with d.find(pc.get_dungeonindex()) begin
			if pc.getqf("dungeonorgindex") != 351 then
			...
			local oldumu = d.getf_from_map_index("razador_dead", n)
			local levelrazador = d.getf_from_map_index("levelrazador", n)

			if oldumu == 1 then
				say_npc()
				say("Razadörü zaten yok etmiþsin.")
				return
			end
			if n == 0 then
				setskin(NOWINDOW)
			...
			if levelrazador == 1 then
				say_npc()
				say("Görünüþe göre razadörden ayrýldýn, sana yardýmcý olmak istiyorum.")
				say_sari("Razadöre devam etmek istiyor musun?")
				if select(locale.yes,locale.no) == 1 then
					pc.setqf("dungeon_enter",1)
					pc.warp(special.razador_dungeon[1][1]* 100, special.razador_dungeon[1][2] * 100,n)
				end
			end
			if levelrazador == 2 then
				say_npc()
				say("Görünüþe göre razadörden ayrýldýn, sana yardýmcý olmak istiyorum.")
				say_sari("Razadöre devam etmek istiyor musun?")
				if select(locale.yes,locale.no) == 1 then
					pc.setqf("dungeon_enter",1)
					pc.warp(special.razador_dungeon[2][1]* 100, special.razador_dungeon[2][2] * 100,n)
				end
			end
		end
